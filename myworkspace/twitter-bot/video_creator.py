...
import time
from google_photos import GooglePhotosDownloader
...
class AutoVideoCreator:
    def __init__(self, script_dir):
        ...
        self.use_google_photos = os.path.exists('credentials.json')  # Auto-detect
        if self.use_google_photos:
            self.google_photos = GooglePhotosDownloader()
        else:
...
"""
    ...
    def copy_images_from_asset(self, output_dir, num_images=10):
        """Copy images from images_asset directory"""
        import glob
        import shutil
        
        asset_dir = "images_asset"
        if not os.path.exists(asset_dir):
            print(f"✗ Asset directory not found: {asset_dir}")
            return []
        
        os.makedirs(output_dir, exist_ok=True)
        source_images = sorted(glob.glob(os.path.join(asset_dir, "*.jpg")))
        ...
        # Copy images
        copied = []
        for i, src in enumerate(source_images[:num_images], 1):
            dst = os.path.join(output_dir, f"{i:02d}_image.jpg")
    ...
    def run(self):
        ...
        # Copy images from asset directory
        images_dir = os.path.join(day_dir, "images")
        print(f"\nCopying images from images_asset...\n")
        
        copied = self.copy_images_from_asset(images_dir, len(keywords))
