...
"""
    ...
    def generate_voiceover_script(self, script_file):
        ...
        markers = ['Host:', 'Narrator:', 'narrators:', '主持人：', '主持人:', 'Host :', 'Narrator :', '旁白：', '旁白:']
        ...
        if not narration:
            ...
            for line in content.split('\n'):
                if chinese_pattern.search(line) and len(line.strip()) > 20:
                    ...
                    if not any(skip in line for skip in ['===', '**', 'SCRIPT', '视频脚本', 'Here is', '[视觉', '视觉效果']):
                        clean = line.strip().strip('"').strip()
                        if clean:
                            narration.append(clean)
        
        # Clean up the narration - remove visual cues and stage directions
        cleaned_narration = []
        for text in narration:
            # Remove visual cues like [视觉效果：...] or [Visuals: ...]
            import re
            text = re.sub(r'\[.*?\]', '', text)  # Remove anything in brackets
            text = re.sub(r'（.*?）', '', text)  # Remove anything in Chinese parentheses
            text = text.strip()
            if text and len(text) > 10:
                cleaned_narration.append(text)
        
        result = '\n\n'.join(cleaned_narration)
        return result if result else ""
    
    def generate_english_voiceover_script(self, script_file):
        """Extract clean English narration for voiceover"""
        with open(script_file, 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Extract the script part (after "SCRIPT:")
        if 'SCRIPT:' in content:
            script_part = content.split('SCRIPT:')[1]
        else:
            script_part = content
        ...
        for line in script_part.split('\n'):
            ...
            if line.startswith('[') or line.startswith('('):
                continue
            if 'Here' in line and 'script' in line:
                continue
            # Keep substantial content lines (lowered threshold to catch "Thank you for watching")
            if len(line) > 10:  # Changed from 30 to 10
                lines.append(line)
    ...
    def save_video_assets(self):
        ...
        # Generate English voiceover script
        print("Generating English voiceover script...")
        english_script = os.path.join(day_dir, "1_english_script.txt")
        english_voiceover = self.generate_english_voiceover_script(english_script)
        if english_voiceover:
            en_vo_file = os.path.join(day_dir, "5_voiceover_english.txt")
            with open(en_vo_file, 'w', encoding='utf-8') as f:
                f.write("ENGLISH VOICEOVER SCRIPT (Clean narration only)\n")
                f.write("="*60 + "\n\n")
                f.write(english_voiceover)
                f.write(f"\n\n{'='*60}\n")
                f.write(f"Word count: {len(english_voiceover.split())}\n")
            print(f"✓ English voiceover script saved")
